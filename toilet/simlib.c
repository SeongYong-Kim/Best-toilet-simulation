...
/* Initialize, update, or report statistics on discrete-time processes:
   ...
   for sampst variable "variable", where "variable":
    ...
    if(variable < 0) { /* Report summary statistics in transfer. */
        ...
        transfer[3] = max[ivar];
        transfer[4] = min[ivar];
        transfer[5] = max[ivar]; /*transfer[5] »ý¼º*/
...
float weibull(float lambda, float k, float r,int stream) /* Weibull variate generation
                                       function. */
{
    return (lambda * pow(fabs(-log(lcgrand(stream))), 1/k))/60 + r;
}

float beta(float a, float b, int stream)

{
    return -a * log(lcgrand(stream));
}

float gamma(float a, float b,int stream)

{
    return (a * pow(fabs(-log(lcgrand(stream))), 1/b))/60 + 0.31673333;
}

float lognormal(float mean, float std, int stream)

{
    return (mean * pow(fabs(-log(lcgrand(stream))), 1 / std))/60;
}

float generalized_pareto(float k, float sigma, float mu, int stream)

{
    return (mu + sigma * (pow(lcgrand(stream), -k) - 1) / k)/60;
}
